...
export const SettingsPage: React.FC<SettingsPageProps> = ({ userInfo, onNavigate }) => {
  ...
  const handleBuyAddon = (packName: string, price: string) => {
      alert(`Proceeding to payment gateway for ${packName} (${price})`);
  };
  ...
  const addonPacks = [
    { name: "Mini Top-Up", renders: 5, price: "₹39", color: "text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20" },
    { name: "Small Pack", renders: 15, price: "₹129", color: "text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20" },
    { name: "Medium Pack", renders: 30, price: "₹199", color: "text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20" },
  ];
             ...
             <div className="bg-white dark:bg-neutral-900 rounded-[20px] p-8 relative z-10">
                ...
                <div className="space-y-3 mb-8">
                    <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-bold text-slate-900 dark:text-white">3 / 5</span>
                    </div>
                    <div className="w-full h-2 bg-slate-100 dark:bg-neutral-800 rounded-full overflow-hidden">
                        <div className="h-full bg-brand-500 w-3/5 rounded-full"></div>
                    </div>
                </div>
             ...
             <div className="bg-white dark:bg-neutral-900 rounded-[20px] p-8">
                ...
                <div className="space-y-4 mb-8">
                    {addonPacks.map((pack, i) => (
                        <div key={i} className="flex items-center justify-between p-4 rounded-xl border border-slate-100 dark:border-neutral-800 hover:border-brand-500/30 transition-all bg-slate-50/50 dark:bg-neutral-800/50">
                            <div>
                                <p className="font-bold text-slate-900 dark:text-white text-sm">{pack.name}</p>
                                <p className={`text-xs mt-0.5 font-medium ${pack.color.split(' ')[0]}`}>{pack.renders} Renders</p>
                            </div>
                            <button 
                                onClick={() => handleBuyAddon(pack.name, pack.price)}
